/*
 * CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr.cpp
 *
...
CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr(float* fb, QNUInt32 sizeof_fb, QNUInt32 lab, CRF_Model* crf, QNUInt32 nodeMaxDur, QNUInt32 prevNode_nLabs, QNUInt32 nextNode_nActualLabs)
...
CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::~CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr()
{
...
/*
 * CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeTransMatrix
 *
 ...
 */
double CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeTransMatrix()
{
	// just for debugging
//	cout << "CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeTransMatrix(): " << endl;
...
//				transMatrixForCurDur[idx]=this->crf_ptr->getFeatureMap()->computeTransMatrixValue(seg_ftr_buf,lambda,plab,lab);
				...
				// pass NULL as the feature vector to computeTransMatrixValue()
				QNUInt32 idx = plab * this->nActualLabs + lab;
				this->transMatrix[idx]=this->crf_ptr->getFeatureMap()->computeTransMatrixValue(NULL,lambda,plab,lab);
...
/*
 * CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeAlpha
 *
 ...
 */
double CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeAlpha()
{
	// just for debugging
//	cout << "CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeAlpha(): " << endl;
...
//			double* prev_adj_seg_alpha = prevAdjacentSeg->getAlpha();
			CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr* prevAdjacentSeg_dnmcast;
			try {
				prevAdjacentSeg_dnmcast = dynamic_cast<CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr*>(prevAdjacentSeg);
				if (prevAdjacentSeg_dnmcast == 0)
				{
					string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeAlpha() caught exception: the previous node is a base class object of CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr.";
					throw runtime_error(errstr);
				}
			}
			catch (exception &e) {
				string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeAlpha() caught exception: "+string(e.what())+", while dynamic casting previous nodes.";
				throw runtime_error(errstr);
			}
...
//			cout << "alphaArray[clab=" << clab << "] = " << this->alphaArray[clab] << endl;
			...
			string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeAlpha() caught exception: "+string(e.what())+", while computing alpha";
			throw runtime_error(errstr);
			return(-1);
		}
	}
...
//	// add transition values to the gathered log-alphas
//
//	// just for debugging
//	cout << "adding transition values to the gathered log-alphas..." << endl;
//
//	// assuming the number of labels in the previous node is equal to that in the current node
//	for (QNUInt32 clab = 0; clab < this->nActualLabs; clab++)
//	{
...
//		for (QNUInt32 plab = 0; plab < this->nActualLabs; plab++)
//		{
//			tmpLogAddAcc[logAddID++] = tmpAlphaArray_gatherPreviousNodes[plab] +
//					this->transMatrix[plab * this->nActualLabs + clab];
//
//			// just for debugging
//			cout << "tmpAlphaArray_gatherPreviousNodes[plab=" << plab << "](" << tmpAlphaArray_gatherPreviousNodes[plab] << ") + " <<
//					"transMatrix[plab=" << plab << ", clab=" << clab << "](" << this->transMatrix[plab * this->nActualLabs + clab] <<
//					") = " << tmpLogAddAcc[logAddID-1] << endl;
//		}
//		try {
//			tmpAlphaArray_gatherPreviousNodes_addTransValue[clab] = logAdd(tmpLogAddAcc,logAddID);
//
//			// just for debugging
//			cout << "tmpAlphaArray_gatherPreviousNodes_addTransValue[clab=" << clab << "]=" << tmpAlphaArray_gatherPreviousNodes_addTransValue[clab] << endl;
//		}
//		catch (exception &e) {
//			string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeAlpha() caught exception: "+string(e.what())+", while computing alpha";
//			throw runtime_error(errstr);
...
//	}
//
//	// add state values to the trans-value-added log-alphas
//
//	// just for debugging
//	cout << "adding state values to the trans-value-added log-alphas..." << endl;
//
//	for (QNUInt32 clab = 0; clab < this->nActualLabs; clab++)
//	{
//		QNUInt32 logAddID = 0;
//		for (QNUInt32 dur = 1; dur <= this->numPrevNodes; dur++)
//		{
//			tmpLogAddAcc[logAddID++] = tmpAlphaArray_gatherPreviousNodes_addTransValue[clab] +
//					this->stateArray[this->nActualLabs * (dur - 1) + clab];
//			this->alphaArray_WithDur[clab*this->nodeLabMaxDur + dur - 1] =
//					tmpAlphaArray_gatherPreviousNodes_addTransValue[clab] +
//					this->stateArray[this->nActualLabs * (dur - 1) + clab];
//
//			// just for debugging
//			cout << "this->alphaArray_WithDur[clab=" << clab << ", dur=" << dur << "] = " <<
//					"tmpAlphaArray_gatherPreviousNodes_addTransValue[clab=" << clab << "](" << tmpAlphaArray_gatherPreviousNodes_addTransValue[clab] <<
//					") + stateArray[clab=" << clab << ", dur=" << dur << "](" << this->stateArray[this->nActualLabs * (dur - 1) + clab] <<
//					") = " << this->alphaArray_WithDur[clab*this->nodeLabMaxDur + dur - 1] << endl;
//		}
//		for (QNUInt32 dur = this->numPrevNodes + 1; dur <= this->nodeLabMaxDur; dur++)
...
/*
 * CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeFirstAlpha
 *
 * Stub function.
 * Should compute the alpha vector for this node for the special case where the node is the first
 * node in the sequence.
 */
double CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeFirstAlpha()
{
	// just for debugging
//	cout << "CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeFirstAlpha()" << endl;
...
/*
 * CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeBeta
 *
 ...
 */
double CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeBeta(double scale)
{
	// just for debugging
//	cout << "CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeBeta(): " << endl;
...
//					<< nextlab << "] = " << this->tmpBetaArray_nextBetasPlusNextStateValue_sumOverDur[nextlab] << endl;
			...
			string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeBeta() caught exception: "+string(e.what())+", while computing beta";
			throw runtime_error(errstr);
			return(-1);
		}
	}
...
//	cout << "adding transition values to the state-value-added log-betas..." << endl;
	...
	if (this->numNextNodes == 0)
	{
		string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeBeta() caught exception: numNextNodes has to be > 0";
		throw runtime_error(errstr);
	}
...
/*
 * CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF
 *
 ...
 */
double CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF(double* ExpF, double* grad, double Zx, QNUInt32 t_next_lab)
{
	// Added by Ryan, just for debugging
//	cout << "CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF(): Very beginning!!" << endl;
...
//	cout << "actualNextLab=" << actualNextLab << endl;
	...
	if (actualLab != CRF_LAB_BAD)
	{
		...
		{
			string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() threw exception: the label is larger than "+ stringify(this->nActualLabs * this->labMaxDur);
			throw runtime_error(errstr);
		}
	...
	if (actualNextLab != CRF_LAB_BAD)
	{
		...
		{
			string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() threw exception: the previous label is larger than "+ stringify(this->nActualLabs * this->labMaxDur);
			throw runtime_error(errstr);
		}
...
//				cout << "clab: " << clab << ", ";
				...
				// just for debugging
//				cout << "alphaArray[clab=" << clab << "](" << this->alphaArray[clab] <<
//						") + transMatrix[clab=" << clab << ", next_lab=" << next_lab << "](" << this->transMatrix[idx] <<
...
//						") - Zx(" << Zx <<
//						") = " << this->alphaArray[clab] + this->transMatrix[idx]+ this->tmpBetaArray_nextBetasPlusNextStateValue_sumOverDur[next_lab] - Zx << endl;
...
//					cout << "transition label match: " << "(" << actualNextLab << "<-" <<  actualLab << ") " << endl;
				...
				if (match)
				{
					logLi+=this->crf_ptr->getFeatureMap()->computeTransExpF(NULL,lambda,ExpF,grad,alpha_beta,actualLab,actualNextLab,clab,next_lab);
				} else {
					logLi+=this->crf_ptr->getFeatureMap()->computeTransExpF(NULL,lambda,ExpF,grad,alpha_beta,CRF_LAB_BAD,CRF_LAB_BAD,clab,next_lab);
				}
			}
		}
...
//			}
//			catch (exception &e) {
//				string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() caught exception: "+string(e.what())+", while computing expected features";
//				throw runtime_error(errstr);
//				return(-1);
//			}
//		}
...
//	cout << "\tAlpha_beta_tot: " << alpha_beta_tot << "\tAlpha_beta_trans_tot: " << alpha_beta_trans_tot << endl;
	...
	if ((alpha_beta_tot >1.000001))  {
		string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() threw exception: Probability sums greater than 1.0 "+stringify(alpha_beta_tot);
		throw runtime_error(errstr);
	}
...
//		string errstr="CRF_StdSegStateNode::computeExpF() threw exception: Probability sums less than 1.0 "+stringify(alpha_beta_tot);
		string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() threw exception: Probability sums less than 0.0 "+stringify(alpha_beta_tot);
		throw runtime_error(errstr);
	}
	else if (alpha_beta_trans_tot > 1.000001) {
		string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() threw exception: Trans Probability sums greater than 1.0 "+stringify(alpha_beta_trans_tot);
		throw runtime_error(errstr);
	}
...
//		string errstr="CRF_StdSegStateNode::computeExpF() threw exception: Trans Probability sums less than 1.0 "+stringify(alpha_beta_trans_tot);
		string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() threw exception: Trans Probability sums less than 0.0 "+stringify(alpha_beta_trans_tot);
		throw runtime_error(errstr);
	}
	...
	if (alpha_beta_tot - alpha_beta_trans_tot > 0.000001) {
		string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() threw exception: State Probability sums(" + stringify(alpha_beta_tot) + ") greater than Trans Probability sum(" + stringify(alpha_beta_trans_tot) + ") for over 0.000001.";
		throw runtime_error(errstr);
	}
	else if (alpha_beta_tot - alpha_beta_trans_tot < -0.000001) {
		string errstr="CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::computeExpF() threw exception: State Probability sums(" + stringify(alpha_beta_tot) + ") less than Trans Probability sum(" + stringify(alpha_beta_trans_tot) + ") for over 0.000001.";
		throw runtime_error(errstr);
	}
...
/*
 * CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::getAlphaPlusTrans(QNUInt32 next_lab)
 *
 * return the summation of [alpha(cur_lab) + trans_value(cur_lab, next_lab)]
 * over all cur_lab for the given next_lab
 *
 */
double CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::getAlphaPlusTrans(QNUInt32 next_lab)
{
...
// Added by Ryan
/*
 * CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::getTransValue(QNUInt32 prev_lab, QNUInt32 cur_lab)
 *
 * Correct getTransValue() function for CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr.
 *
 */
double CRF_StdSegStateNode_WithoutDurLab_WithoutTransFtr::getTransValue(QNUInt32 prev_lab, QNUInt32 cur_lab)
{
