...
[ -f $conf/enh_spk.list ] || error_exit "$PROG: Enhanced eval-set speaker list not found.";
...
if $uppercased; then
  ...
  tr '[:lower:]' '[:upper:]' < $conf/enh_spk.list > $tmpdir/enh_spk
  ...
  tr '[:upper:]' '[:lower:]' < $conf/enh_spk.list > $tmpdir/enh_spk
  ls -d "$*"/train/dr*/* | sed -e "s:^.*/::" > $tmpdir/train_spk
fi
