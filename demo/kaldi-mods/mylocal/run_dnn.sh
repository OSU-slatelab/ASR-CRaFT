...
if [ $stage -le 0 ]; then
  ...
  dir=$data_fmllr/enh
  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
     --transform-dir $gmmdir/decode_enh \
     $dir data/enh $gmmdir $dir/log $dir/data || exit 1
  # train
...
if [ $stage -le 2 ]; then
  ...
  steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" --acwt 0.2 \
    $gmmdir/graph $data_fmllr/enh $dir/decode_enh || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  for ITER in 1 6; do
    ...
    steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" \
      --nnet $dir/${ITER}.nnet --acwt $acwt \
      $gmmdir/graph $data_fmllr/enh $dir/decode_enh_it${ITER} || exit 1
  done 
fi
