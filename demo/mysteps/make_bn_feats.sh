...
if [ $# != 5 ]; then
   ...
   echo "options: "
   echo "  --use-prior                                      # whether to divide by target prior"
   echo "  --htk-save                                       # whether to save htk format as well"
...
mkdir -p $data || exit 1;
cp $srcdata/* $data 2>/dev/null; rm -f $data/feats.scp $data/cmvn.scp;
...
[ "$use_prior" == "true" ] && prior_opt="--no-softmax=true --class-frame-counts=$nndir/ali_train_pdf.counts" || prior_opt=
...
$cmd JOB=1:$nj $logdir/make_bnfeats.JOB.log \
  nnet-forward $prior_opt $nnet "$feats" \
...
if [ $htk_save ]; then
  ...
  $cmd JOB=1:$nj $logdir/htk_copy_bnfeats.JOB.log \
    copy-feats-to-htk --output-dir=$bnfeadir/htk --output-ext=fea scp:$data/split$nj/JOB/feats.scp || exit 1
  echo "DONE!"
fi
