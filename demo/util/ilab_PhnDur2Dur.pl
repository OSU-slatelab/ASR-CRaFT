...
if ($tag ne "NUML") {die "Error: The header format of the mapping file is incorrect.";}

$header=<MAPFILE>;
chomp($header);
($tag, $splitLongSeg)=split('\s+',$header);
if ($tag ne "SPLITLONGSEG") {die "Error: The header format of the mapping file is incorrect.";}

$header=<MAPFILE>;
chomp($header);
($tag, $maxDur)=split('\s+',$header);
if ($tag ne "MAXDUR") {die "Error: The header format of the mapping file is incorrect.";}

$header=<MAPFILE>;
chomp($header);
($tag, $groupSize)=split('\s+',$header);
if ($tag ne "GROUPSIZE") {die "Error: The header format of the mapping file is incorrect.";}

$header=<MAPFILE>;
chomp($header);
($tag, $CUTOFF)=split('\s+',$header);
if ($tag ne "CUTOFF") {die "Error: The header format of the mapping file is incorrect.";}
